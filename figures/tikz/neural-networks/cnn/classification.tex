...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%   
\node[canvas is zy plane at x=0] (image) at (0,0,0) {\includegraphics[width=12cm,height=12cm]{img/sub-img/landscape-example.png}};


% CONVOLUTION
\pic[shift={(4,0,0)}] at (image) {RightBandedBox={name=c1,caption=CONVOLUTION,
...
\node[scale=2] (pouet) at (29,-0.5,0) {Softmax};

\node[scale=3, inner sep=0pt] (tab) at (33.5,0,0) {
    \begin{tabular}{|c|}
        \hline
        Forêt \\
        \hline
...
\draw [connection]  (up2-east)   -- node {\midarrow} (tab.west);
\draw[densely dashed]
(c5-nearnortheast) -- (tab.north west)
(c5-nearsoutheast) -- (tab.south west)
(c5-farsoutheast)  -- (tab.south west)
(c5-farnortheast)  -- (tab.north west)
;
\end{tikzpicture}
\end{document}
