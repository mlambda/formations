...
%Start drawing the thing...    
    ...
    \node [function] (ibox1) at (-2,-0.75) {$\sigma$};
   ...
   % Draw opérators   named mux# , add# and func#
    ...
    \node[hencoder, label={[mylabel]Accumulateur}] (c) at (-4,1.5){$\vec{c}_{t-1}$};
    \node[hencoder, label={[mylabel]Hidden}] (h) at (-4,-1.5)     {$\vec{h}_{t-1}$};
    \node[input,    label={[mylabel]left:Input}] (x) at (-2.5,-3) {$\vec{x}_{t  }$};

    % Draw External outputs? named as basis c2,h2,x2
    \node[hencoder, label={[mylabel]Accumulateur}] (c2) at (4,1.5)      {$\vec{c}_{t}$};
    \node[hencoder, label={[mylabel]Hidden}] (h2) at (4,-1.5)     {$\vec{h}_{t}$};
    \node[output,   label={[mylabel]left:Output}] (x2) at (2.5,3) {$\vec{o}_{t}$};
